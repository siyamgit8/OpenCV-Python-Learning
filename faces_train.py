import os
import cv2 as cv
import numpy as np

people = []
for i in os.listdir(r'D:\Python\OpenCV\cv photos\train'): # here r is rerpresenting raw file driectory 
    people.append(i)
print(people)
# people = ['Ben Afflek', 'Elton John', 'Jerry Seinfield', 'Madonna', 'Mindy Kaling']

DIR = r'D:\Python\OpenCV\cv photos\train'

haar_cascade = cv.CascadeClassifier('haar_face.xml')

feature = []
labels = []

def create_train():
    for person in people :
        path = os.path.join(DIR , person )
        label = people.index(person)

        for img in os.listdir(path):
            img_path = os.path.join(path , img)

            img_array = cv.imread(img_path)
            gray = cv.cvtColor(img_array , cv.COLOR_BGR2GRAY)

            faces_rect = haar_cascade.detectMultiScale(gray , scaleFactor= 1.1 , minNeighbors= 4)

            for (x , y ,w ,h) in faces_rect:
                faces_roi = gray[y:y+h , x :x +w]
                feature.append(faces_roi)
                labels.append(label)

create_train()
print('Training Done')


feature = np.array(feature ,dtype= object )
labels = np.array(labels)

print('features length =' , len(feature))
print('labels length=' , len(labels))

face_recognizer = cv.face.LBPHFaceRecognizer_create() # this function will 
# Train the Recognizer on the feature list and label list 
face_recognizer.train(feature , labels)

face_recognizer.save('face_trained.yml')


np.save('features.npy' , feature)
np.save('labels.npy' , labels)